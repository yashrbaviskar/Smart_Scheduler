...
export default function ChatPage() {
  ...
  useEffect(() => {
    const socketIo = io({ path: "/api/socketio" })
    setSocket(socketIo)
    socketIo.on("receive_message", (msg: any) => {
      setUserMessages((prev) => [...prev, msg])
    })
    socketIo.on("ai_intent", (data: any) => {
      setAiIntent(data.summary)
    })
    return () => {
      socketIo.disconnect()
    }
  ...
  const handleUserSend = (e: React.FormEvent) => {
    e.preventDefault()
    if (!userInput.trim() || !selectedConversation) return
    const msg = {
      conversationId: selectedConversation,
      senderName: user?.firstName || "User",
      content: userInput,
      thread: [...userMessages, { senderName: user?.firstName || "User", content: userInput }],
    }
    socket?.emit("send_message", msg)
    setUserMessages((prev) => [...prev, { senderName: user?.firstName || "User", content: userInput }])
    setUserInput("")
  }
            ...
            <Card className="h-full">
                        ...
                        <>
                          ...
                          <ScrollArea className="flex-1 px-6 py-4">
                            <div className="space-y-4">
                              {userMessages.map((msg, idx) => (
                                <div
                                  ...
                                  className={`flex gap-3 ${
                                    msg.senderName === user?.firstName ? "justify-end" : "justify-start"
                                  }`}
                                >
                              ...
                              {aiIntent && (
                                <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 border rounded-lg">
                                  <div className="flex items-center gap-2 mb-2">
                                    <Sparkles className="h-4 w-4 text-blue-600" />
                                    <span className="font-semibold text-blue-900">AI Scheduling Insight</span>
                                  </div>
              ...
              <CardFooter className="border-t">
                {activeTab === "ai-assistant" ? (
                  ...
                  <form className="flex w-full space-x-2" onSubmit={handleUserSend}>
                    <Input
                      value={userInput}
                    ...
                    />
                    <Button type="submit" disabled={!selectedConversation || !userInput.trim()}>
                      <Send className="h-4 w-4" />
                    </Button>
                  </form>
                )}
